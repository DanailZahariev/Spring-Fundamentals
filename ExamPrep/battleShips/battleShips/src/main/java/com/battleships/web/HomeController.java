...
public class HomeController {
    ...
    public String index(Model model) {
        ...
        List<ShipViewModel> allShips = shipService.findAll();

        model.addAttribute("currentUser", currentUser);
        model.addAttribute("attacker", allShips.stream().filter(s -> s.getUser().getId().equals(currentUser.getId())).collect(Collectors.toList()));
        model.addAttribute("defender", allShips.stream().filter(s -> !s.getUser().getId().equals(currentUser.getId())).collect(Collectors.toList()));
    ...
    @PostMapping("/home")
    public String shipFight(@Valid HomeBindingModel homeBindingModel, BindingResult bindingResult,
